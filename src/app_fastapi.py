...
# =========================
# 농업/영농 관련성 가드: 키워드 로딩 & 판정
# =========================
_DEFAULT_FARM_KEYWORDS: Set[str] = {
    "영농","농업","농사","작물","밭","포장","하우스","과원",
    "관수","灌水","시비","비료","엽면시비","방제","제초",
    "파종","정식","수확","적과","전정","멀칭",
    "농약","REI","PHI","병해","해충","진딧물","총채","탄저","역병",
    "배추","고추","사과","토마토","감자","상추","딸기","파프리카","오이","참외","포도","복숭아",
}

def _load_farm_keywords(path: str) -> Set[str]:
    """
    kb/farming_keywords.txt 에서 키워드 로드.
    - 줄 단위 파싱, '#' 이후 주석 제거
    - 쉼표/공백 구분자 모두 허용
    - 빈 줄 무시
    """
    ...
    try:
        ...
        # 파일 파싱 실패 시 빈 세트 반환 → 호출부에서 폴백
        return set()
    return kws

def _is_farming_related(text: str) -> bool:
    """
    텍스트가 영농/농업과 관련 있는지 간단 판정.
    - kb 키워드 파일을 우선 사용, 없으면 기본 키워드 사용
    - 부분 문자열 매칭(대소문자 무시)
    """
    if not text:
        return False
    # 캐시된 세트 사용
    keywords: Set[str] = getattr(app.state, "farm_keywords", set())
    if not keywords:
        keywords = _load_farm_keywords(KEYWORDS_PATH)
        if not keywords:
            keywords = set(_DEFAULT_FARM_KEYWORDS)
        app.state.farm_keywords = keywords

    t_low = text.lower()
    for k in keywords:
        if not k:
            continue
        if (k in text) or (k.lower() in t_low):
            return True
    return False
...
def _startup():
    ...
    # 3) 키워드 선로딩(실패해도 런타임에서 폴백)
    try:
        app.state.farm_keywords = _load_farm_keywords(KEYWORDS_PATH) or set(_DEFAULT_FARM_KEYWORDS)
    except Exception:
...
def summarise(req: SummariseRequest):
    # 가드
    if not _is_farming_related(req.stt_text):
...
def summarise_file(req: SummariseFileRequest):
    ...
    # 가드
    if not _is_farming_related(stt_text):
        return PlainTextResponse(REJECT_MSG)
...
def ingest(req: IngestRequest):
    ...
    try:
        ...
        _pipeline = None
        app.state.vector_backend = backend
        # 키워드 파일 경로가 바뀌었을 수도 있으므로 재로딩 기회 제공(경로는 동일하더라도 최신화)
        try:
            app.state.farm_keywords = _load_farm_keywords(KEYWORDS_PATH) or set(_DEFAULT_FARM_KEYWORDS)
        except Exception:
            app.state.farm_keywords = set(_DEFAULT_FARM_KEYWORDS)
        return {"status": "ok", "kb_dir": kb_dir, "vector_backend": backend, "keywords_count": len(app.state.farm_keywords)}
    except Exception as e:
